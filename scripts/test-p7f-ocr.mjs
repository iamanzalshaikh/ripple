/**
 * P7f manual test — OCR on the foreground window via sidecar WinRT.
 *
 ...
 *   3. Focus a window with visible text (Notepad, browser, etc.)
 *   4. node scripts/test-p7f-ocr.mjs
 */
    ...
    const onData = (chunk) => {
      ...
      for (const frame of frames) {
        if (frame.error === "already_connected") {
          ...
          reject(
            new Error(
              "already_connected — npm run dev (Electron) owns the sidecar pipe. Stop dev first.",
            ),
          );
          return;
        }
...
async function main() {
  ...
  if (!existsSync(sessionPath)) {
    console.error(`No session file — start sidecar first: ${sessionPath}`);
    process.exit(1);
  }
  ...
  if (!caps.ocr) {
    console.error("Sidecar reports ocr=false — install Windows OCR language pack or rebuild");
    process.exit(1);
  }

  console.log("OCR in 2s — focus a window with visible text...");
  ...
  const result = await rpc(sock, "screenshot_ocr", {});
  const ms = Math.round(performance.now() - t0);

  console.log(`OCR result (${ms}ms, ${result.width}x${result.height}, ${result.lineCount} lines):`);
  console.log("---");
